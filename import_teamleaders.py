...
def create_team_leader(first_name, last_name, email):
    try:
        ...
        # Добавление пользователя в группу "Ответственные"
        team_leaders_group, created = Group.objects.get_or_create(name='Ответственные')
        user.groups.add(team_leaders_group)
        
        # Создание профиля тимлидера
        TeamLeaderProfile.objects.create(user=user)
        
        print(f"Добавлен ответственный (пользователь): {first_name} {last_name} ({email})")
        return True
    
    except Exception as e:
        print(f"Ошибка при добавлении ответственного (пользователя) {first_name} {last_name}: {str(e)}")
        return False
...
def import_team_leaders():
    # Список ответственных для импорта (имена и фамилии поменяны местами и переведены на английский)
    team_leaders_data = [
    ...
    for first_name, last_name, email in team_leaders_data:
        ...
        if scanner:
            print(f"Найден сканер: {scanner.first_name} {scanner.last_name} - будет добавлен как Ответственный")
        ...
        if existing_teamleader:
            print(f"Ответственный уже существует в новой модели: {first_name} {last_name}")
            ...
            if not existing_teamleader.scanner and scanner:
                ...
                print(f"Обновлена связь со сканером для Ответственного: {first_name} {last_name}")
        else:
            ...
            # Сохраняем Ответственного (при сохранении автоматически создастся сканер, если его нет)
            new_teamleader.save()
            print(f"Добавлен Ответственный в новую модель: {first_name} {last_name}")
            added_count += 1
        
        # Создаем пользователя-Ответственного, если его еще нет
        success = create_team_leader(first_name, last_name, email)
        if not success:
            print(f"Пользователь-Ответственный уже существует или не удалось создать: {first_name} {last_name}")
    
    print("\nРезультаты импорта Ответственных:")
    print(f"Добавлено Ответственных в новую модель: {added_count}")
