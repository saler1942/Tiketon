    ...
    try:
        ...
        for full_name in names_column:
            ...
            if len(name_parts) >= 2:
                ...
                base_email = f"{first_name.lower()}.{last_name.lower().replace(' ', '.')}@scanner.com"
                email = base_email
                
                # Проверка, существует ли уже сканер с таким email
                counter = 1
                while Scanner.objects.filter(email=email).exists():
                ...
                if existing_scanner:
                    ...
                    print(f"Добавлен: {first_name} {last_name} ({email})")
                    added_count += 1
            else:
